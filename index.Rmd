...
<script>
  ...
  // Add aria-labels to slider control containers to inform the user as to their purpose
  var arrSliders = document.getElementsByClassName('crosstalk-input-slider');   // The div surrounding the control
  var sliderGroupName;
  
  for (var i = 0; i < arrSliders.length; i++) {
    // The first child node is the span containing the heading for the slider.
    // Use this heading as part of the aria-label to increase understanding
    sliderGroupName = 'Slider range control for ' + arrSliders[i].childNodes[1].textContent;
    arrSliders[i].setAttribute('aria-label', sliderGroupName);
    
    arrSliders[i].setAttribute('tabindex', '0');    // allow the slider group to be accessed via tab
  }
